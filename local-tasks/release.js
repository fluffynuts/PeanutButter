var fs = require("fs"), gulp = require("gulp"), runSequence = requireModule("run-sequence"),
  msbuild = require("gulp-msbuild"), sleep = requireModule("sleep"), del = require("del"), {
    ls,
    FsEntities
  } = require("yafs"), exec = requireModule("exec"), spawn = requireModule("spawn"), es = require("event-stream"),
  path = require("path"), findTool = requireModule("testutil-finder").findTool, PQueue = require("p-queue").default,
  env = requireModule("env"), gutil = requireModule("gulp-util"), { envFlag } = requireModule("env-helpers"),
  usingDotnetCore = env.resolveFlag("DOTNET_CORE"), commonConfig = {
    toolsVersion: "auto", stdout: true, verbosity: "minimal", errorOnFail: true, architecture: "x64"
  };
...
const pushNugetPackages = usingDotnetCore ? pushNugetPackagesWithDotNet : pushNugetPackagesWithNugetExe
...
function findProjectNextTo(nuspec) {
  const dir = path.dirname(nuspec), contents = fs.readdirSync(dir),
...
gulp.task("update-project-nugets", async () => {
  const nugets = await ls("source", { recurse: true, entities: FsEntities.files, match: /nuget.exe/i });
  const localNuget = await findLocalNuget()
