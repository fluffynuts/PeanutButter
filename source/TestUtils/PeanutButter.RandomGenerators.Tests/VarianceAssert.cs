...
{
    public static void IsVariant<TObject, TProperty>(
    ...
    )
    {
        ...
        {
            throw new Exception(
                string.Join("", "Unable to find property '", propertyName, "' on type '", type.Name)
            );
        }
        ...
        catch (Exception ex)
        {
            Assert.Fail(
                string.Join(
                    "",
                    "Unable to get list of property values for '",
                    propertyName,
                    "' of type '",
