...
public
#endif
    class UnableToCreateDynamicBuilderException : Exception
{
    /// <summary>
    ...
    public UnableToCreateDynamicBuilderException(Type type, TypeLoadException typeLoadException) :
        base(
            $"Unable to create dynamic builder for type {type.PrettyName()}. If {type.PrettyName()} is internal, you should make InternalsVisibleTo \"PeanutButter.RandomGenerators.GeneratedBuilders\". If {type.PrettyName()} is nested, ensure that all parents are public.",
            typeLoadException
        )
    {
        Type = type;
